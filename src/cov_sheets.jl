...
mutable struct AzCov{T, d, spin, Fplan<:AbstractFFTs.ScaledPlan, U}
    ...
    function AzCov(::Type{T}, filenm::String, nside::Int; d::Int=2, spin::Int=0) where T<:Real
        ...
        cs = new{T, d, spin, typeof(Urow), typeof(kidx_blk)}(filenm, jld2file, Urow, nside, nθx, nφx, nφk, T.(θ), T.(φᵀ),  kidx_blk, Lsheet_names, lower_tri_Idx)       
...
function AzCov(covf, θ::AA, φᵀ::AA, kidx_blk, nside; d::Int=2, spin::Int=0) where {T, AA<:Array{T,1}}
    ...
    @showprogress for (i,k) ∈ enumerate(kidx_blk)
        ## ΣTT = CMBrings.Σsheets_k(θ, k, covf, φᵀ)
        ΣTT = CMBrings.shared_Σsheets_k(θ, k, covf, φᵀ)
    ...
    write(jld2file, "nside", nside)
    write(jld2file, "θ", θ)
    write(jld2file, "φᵀ", φᵀ)
    write(jld2file, "kidx_blk", kidx_blk)
    write(jld2file, "lower_tri_Idx", lower_tri_Idx)

    close(jld2file)

    return AzCov(T, filenm, nside; d=d, spin=spin)
end
...
function pixunitary(::Type{T}, nθx, nφx, nside) where {T<:Number} 
    randn(T, nθx, nφx)
end
