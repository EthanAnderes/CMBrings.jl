...
θ, φ, Ω, Δθ, nθ, nφ, freq_mult, grid_type = @sblock let 
    ...
    φ      = CirculantCov.fraccircle(φspan[1], φspan[2], 768)

    θ, θ∂, grid_type = CMBrings.θ_grid(; θspan=(2.6,2.8), N= 3500, type=:equiθ)
    ## θ, θ∂, grid_type = CMBrings.θ_grid(; θspan=(2.6,2.8), N=1200, type=:healpix)
    ## θ, θ∂, grid_type = CMBrings.θ_grid(; θspan=(2.6,2.9), N=1024, type=:healpix)
    ...
    ## θ, θ∂, grid_type = CMBrings.θ_grid(; θspan=(2.6,2.9), N= 2372, type=:equicosθ)
    ## θ, θ∂, grid_type = CMBrings.θ_grid(; θspan=(2.5,2.7), N= 3900, type=:equiθ)
    θspan = (θ∂[1], θ∂[end])

    nθ, nφ = length(θ), length(φ)
    Ω  = CirculantCov.counterclock_Δφ(φ[1], φ[2]) .* diff(.- cos.(θ∂))
    Δθ = diff(θ∂)
...
# EB▪tmp, Phi▪tmp, Beam▪tmp, N▪tmp, Ð▪tmp⁻¹, NΦN▪tmp  = @sblock let ℓ, eeℓ, bbℓ, ẽẽℓ, b̃b̃ℓ, ϕϕℓ, beamℓ, NΦNℓ, μK_arcmin, θ, φ, freq_mult, Ω 
...
#     Ð▪⁻¹ =  CircOp(@. sqrt(Hermitian(EB▪)) / sqrt(Hermitian(ẼB̃▫ + 4*N▪)))
...
EB▪, Phi▪, Beam▪, N▪, Ð▪⁻¹, NΦN▪  = @sblock let ℓ, eeℓ, bbℓ, ẽẽℓ, b̃b̃ℓ, ϕϕℓ, beamℓ, NΦNℓ, μK_arcmin, θ, φ, freq_mult, Ω 
    ...
    for k = 1:nθ
        for j = 1:nθ
            ...
            Phiγⱼₖ  = CirculantCov.γⱼₖℓ⃗(θ[j], θ[k], φ, covβPhi,  ptmW)
            NΦNγⱼₖ  = CirculantCov.γⱼₖℓ⃗(θ[j], θ[k], φ, covβNΦN,  ptmW)
            Beamγⱼₖ = CirculantCov.γⱼₖℓ⃗(θ[j], θ[k], φ, covβBeam, ptmW)
            EBγⱼₖ, EBξⱼₖ = CirculantCov.γⱼₖℓ⃗_ξⱼₖℓ⃗(θ[j], θ[k], φ, covβEB, ptmW)
            ẼB̃γⱼₖ, ẼB̃ξⱼₖ = CirculantCov.γⱼₖℓ⃗_ξⱼₖℓ⃗(θ[j], θ[k], φ, covβẼB̃, ptmW)
...
# 7mins vrs 4mins, slower uses CirculantCov.γⱼₖℓ⃗_ξⱼₖℓ⃗ ...
