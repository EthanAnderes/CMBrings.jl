...
θ, φ, Ω, Δθ, nθ, nφ, freq_mult, tmUS2, tmUS0 = @sblock let 
    ...
    freq_mult = 3 # 3
    nθ, nφ    = (300, 768)
    θnorth∂ = 2.6 # 2.5 #  2.3784 # 
    θsouth∂ = 2.8 # 2.7 #  2.7694 # 
...
EB▪, Phi▪, Beam▪, N▪, Ð▪⁻¹, NΦN▪  = @sblock let ℓ, eeℓ, bbℓ, ẽẽℓ, b̃b̃ℓ, ϕϕℓ, beamℓ, NΦNℓ, μK_arcmin, θ, φ, freq_mult, Ω 
    ...
    fβp      = (β,p) -> p * (sin(π*β/p - π/2) + 1) / 2
    β_grid   = range(0,π,length=100_000) .|> β -> fβp(β, π)
    covβEB   = βcovSpin2(ℓ, eeℓ, bbℓ; β_grid)
    covβẼB̃   = βcovSpin2(ℓ, ẽẽℓ, b̃b̃ℓ; β_grid)
    covβPhi  = βcovSpin0(ℓ, ϕϕℓ; β_grid)
    covβNΦN  = βcovSpin0(ℓ, NΦNℓ; β_grid)
    covβBeam = βcovSpin0(ℓ, beamℓ; β_grid)
...
function generate_∇!_∇!ϕ_1storder(θ::Vector{Tf}, φ::Vector{Tf}) where {Tf}
    Δφ = φ[2] - φ[1]
    Δθ = vcat(diff(θ), θ[end]-θ[end-1])
    # Δθ, Δφ = θ[2] - θ[1], φ[2] - φ[1]
